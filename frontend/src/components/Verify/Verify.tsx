...
// ... existing code ...
useEffect(() => {
  // Retrieve countdown value from cookie
...
// ... existing code ...
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(apiConfig.user.verify, data, {
        headers: {
          ...
          <div>
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col items-center">
                <div className="mb-6">
                  Nhập mã xác thực OTP gửi đến <span className="font-bold">{userData ? userData.email : 'Email không có sẵn'}</span>
                </div>
                <div>
